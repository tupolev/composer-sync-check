...
class ComposerSyncProjectService(private val project: Project) : Disposable {
    ...
    fun runComposerInstallFromToolWindow() {
        ...
        runner.run(
            ...
            onStarted = {
                ...
                console.setRunInstallEnabled(false)
            ...
            onFinished = {
                console.setRunInstallEnabled(true)
    ...
    private fun runCheck(source: String) {
        ...
        AppExecutorUtil.getAppExecutorService().execute {
            ...
            if (ignoredForSession) return@execute
            if (!settings.state.showNotificationBalloon) return@execute
            if (lastNotificationFingerprint == result.fingerprint) return@execute
            ...
            notifications.notifyOutOfSync(
                onRunInstall = {
                    ...
                    runner.run(
                        command = settings.state.composerCommand,
                        workingDirectory = resolvedFiles.workingDirectory,
                        onStarted = {
                            composerCommandRunning = true
                            console.setRunInstallEnabled(false)
                            refreshStatusIndicator()
    ...
    private fun refreshStatusIndicator() {
        val status = when {
            composerCommandRunning -> ComposerSyncConsoleService.SyncStatus.RUNNING
